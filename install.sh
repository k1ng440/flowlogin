...
sudo -u "$SERVICE_USER" "$UV_BIN" pip install --python 3.11 patchright httpx 2>/dev/null || true

echo "[*] Installing patchright chromium..."
sudo -u "$SERVICE_USER" "$UV_BIN" run --with patchright patchright install chromium
